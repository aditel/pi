...
function getTemperatureSensors(request, response) {
    connection.query("SELECT sid s, temp t, humid h FROM (SELECT * FROM th ORDER BY sid, ts DESC) th GROUP BY s", function(err, results, fields) {
...
function getLatestData(request, response) {
    ...
    connection.query("SELECT ts AS ts, temp AS t, humid AS h FROM th WHERE sid=" + data.sid + " ORDER BY ts DESC LIMIT 20", function(err, results, fields) {
	results = results.filter(function(elem, index, array) {
	    return true;//index % 5 == 0;
